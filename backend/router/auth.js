...
;
  ...
  // //////----------------verify email of user --------------------------------------/////////-------->
        ...
        if(!user){ return  res.status(400).json({ error:'Email is not register Please Signup'});}

    //////////--------- verify password of user -------------------------------------------------------->

    const comparePass= await bcrypt.compare(password,user.password);
    if(!comparePass){ return res.status(400).json({error:'Incorrect password'});}
  ...
  router.get('/getuser',fetUser, async(req,res)=>{
    try{
...
body('password').isLength({ min: 5 })
  ...
  /////////////////-----------display error for for duplicate email-------////////////////////------->
      ...
      try{
        ...
        if(!admin){return  res.status(400).json("not allowed");}
